...
public class PayOrderServiceImpl extends BaseService implements IPayOrderService {
    ...
    public String doWxPayReq(String tradeType, JSONObject payOrder, String resKey) {
        ...
        if(s == null) {
            return XXPayUtil.makeRetData(XXPayUtil.makeRetMap(PayConstant.RETURN_VALUE_SUCCESS
                    , "", PayConstant.RETURN_VALUE_FAIL, "0111", "调用微信支付失败"), resKey);
        }
        ...
        String nofiyResult =XXPayUtil.makeRetData(map, resKey);
        //记录日志
        payLogService.updatePayLog(nofiyResult,payOrder,2);
        return nofiyResult;
    }
    ...
    public String doAliPayReq(String channelId, JSONObject payOrder, String resKey) {
        ...
        String nofiyResult =XXPayUtil.makeRetData(map, resKey);
        //记录日志
        payLogService.updatePayLog(nofiyResult,payOrder,1);
        return nofiyResult;
    }
    ...
    public Map createPayOrder(String jsonParam) {
        ...
        //初始状态
        payOrder.setStatus(PayConstant.PAY_STATUS_INIT);
        //TODO 初始化通知次数
        payOrder.setNotifyCount(PayConstant.PAY_STATUS_INIT);
    ...
    @Override
    public int validateMchOrderNo(String mchOrderNo) {
        return super.baseValidateMchOrderNo(mchOrderNo);
    }
}
