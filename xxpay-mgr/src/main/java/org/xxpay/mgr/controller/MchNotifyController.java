...
public class MchNotifyController {
    ...
    private static  String  notifyIp="http://10.43.112.152:3020";
    ...
    @RequestMapping(value = "/mchNotify", method = RequestMethod.GET)
    @ResponseBody
    public String  mchNotify(String payOrderId) {
        try{
            MchNotify item = mchNotifyService.selectMchNotify(payOrderId);
            if(item.getStatus()==2){
                return "2";
            }
            String result="";
            ...
            if(payOrderId!=null&&payOrderId!=""){
                String url=notifyIp+"/api/mch/doMchNotify?";
                result = XXPayUtil.call4Post(url + reqData);
            }
            return "1";
        }catch (Exception e){
            return  "0";
